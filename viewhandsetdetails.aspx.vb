...
Partial Class viewhandsetdetails
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        con.Open()
        p = Request.QueryString("handsetid")
    ...
    Public Function getdataset() As DataSet
        ...
        Dim con As New SqlConnection(strcon)
        con.Open()
        cmd = New SqlCommand("select * from adminhandset where handsetname='" + p + "'", con)
        drs = cmd.ExecuteReader
        While drs.Read
            If drs("copies") = 0 Then
                lblstock.Text = "Out Of Stock"
                lblstock.ForeColor = Drawing.Color.Red
    ...
    Public Sub bindgrid(ByVal ds As DataSet)
        Repeater1.DataSource = ds.Tables("adminhandset")
        Repeater1.DataBind()
    End Sub
    ...
    Protected Sub btnAddToCart_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddToCart.Click
        ...
        If Session("username") <> "" Then
            ...
            Else
                Dim s5 As String
                s5 = "1"
                cmd = New SqlCommand("select * from cartdetails", con)
                drs = cmd.ExecuteReader()
                While drs.Read()
            ...
            If flag = 1 Then
                Dim s5 As String
                s5 = "1"
                cmd = New SqlCommand("insert into cartdetails values('" + s + "','" + Session("orderid") + "','" + a5 + "','" + s3 + "','" + s5 + "','" + s3 + "')", con)
                cmd.ExecuteNonQuery()
                Session("dis") = 1
                Response.Redirect("cartdetails.aspx")
            End If
